 ...
 *
 * A polymorphic link/button component with animated underline hover effects.
 * Renders as <a> by default, but pass `as="button"` to use it for buttons
 * (e.g. scroll-to-section triggers) while keeping the same visual effect.
 * Pass `injectStyle={false}` when the styles are already injected once
 * elsewhere on the page (recommended when rendering many instances).
 *
 * @example
 * <LineHoverLink variant="slide" href="/about">Learn more</LineHoverLink>
 * <LineHoverLink as="button" variant="slide" onClick={fn}>About</LineHoverLink>
 */
export const LineHoverLink = React.forwardRef(
  (
    { as = "a", variant = "slide", children, className, injectStyle = true, ...props },
    ref
  ) => {
    const Tag = as;
    ...
    return (
      <>
        {injectStyle && <style>{lineHoverStyles}</style>}
        <Tag
          ref={ref}
